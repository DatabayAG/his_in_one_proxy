...
class ParseAccountsTest  extends TestCaseExtension{
	...
	public function test_parse_shouldReturnAccount()
	{
		...
		$this->assertEquals('max.mueller', $account[0]->getUserName());
		$this->assertEquals('21', $account[0]->getAccountAuthId());
		$this->assertEquals('LDAP', $account[0]->getAuthInfo());
		$this->assertEquals('ecs-43', $account[0]->getExternalSystemId());
		$this->assertEquals('0', $account[0]->isLdapAccount());
		$this->assertEquals('3654775', $account[0]->getOrgUnitLid());
	...
	public function test_parse_shouldReturnAccounts()
	{
		...
		$this->assertEquals('helen.schmidt', $account[1]->getUserName());
		$this->assertEquals('212', $account[1]->getAccountAuthId());
		$this->assertEquals('Shibboleth', $account[1]->getAuthInfo());
		$this->assertEquals('ecs-432', $account[1]->getExternalSystemId());
		$this->assertEquals('1', $account[1]->isLdapAccount());
		$this->assertEquals('3654775', $account[1]->getOrgUnitLid());
