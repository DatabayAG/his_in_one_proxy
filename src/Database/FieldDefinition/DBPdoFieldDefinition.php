...
namespace HisInOneProxy\Database\FieldDefinition;
use Exception;
use HisInOneProxy\Database\DbPdo;

abstract class DBPdoFieldDefinition
{
    public array $lobs;
    ...
    protected static \DBPdoFieldDefinition $instance;
    /**
     * @var string[][]
     */
    ...
    /**
     * DBPdoFieldDefinition constructor.
     */
    public function __construct(DbPdo $ilDBInterface)
    {
    ...
    public function checkTableName(string $table_name): bool
    {
        if (!preg_match(self::DEFINITION_TABLE_NAME, $table_name)) {
            throw new Exception('Table name must only contain _a-z0-9 and must start with a-z.');
        }

        if ($this->isReserved($table_name)) {
            throw new Exception("Invalid table name '" . $table_name . "' (Reserved Word).");
        }

        if (stripos($table_name, "sys_") === 0) {
            throw new Exception("Invalid table name '" . $table_name . "'. Name must not start with 'sys_'.");
        }

        if (strlen($table_name) > 22) {
            throw new Exception("Invalid table name '" . $table_name . "'. Maximum table identifer length is 22 bytes.");
        }
    ...
    public function checkColumnName(string $column_name): bool
    {
        ...
        if ($this->isReserved($column_name)) {
            throw new Exception("Invalid column name '" . $column_name . "' (Reserved Word).");
        }

        if (stripos($column_name, "sys_") === 0) {
            throw new Exception("Invalid column name '" . $column_name . "'. Name must not start with 'sys_'.");
        }

        if (strlen($column_name) > 30) {
            throw new Exception("Invalid column name '" . $column_name . "'. Maximum column identifer length is 30 bytes.");
        }
    ...
    public function checkIndexName(string $a_name): bool
    {
        if (!preg_match("/^[a-z]+[_a-z0-9]*$/", $a_name)) {
            throw new Exception("Invalid column name '" . $a_name . "'. Column name must only contain _a-z0-9 and must start with a-z.");
        }

        if ($this->isReserved($a_name)) {
            throw new Exception("Invalid column name '" . $a_name . "' (Reserved Word).");
        }

        if (strlen($a_name) > 3) {
            throw new Exception("Invalid index name '" . $a_name . "'. Maximum index identifer length is 3 bytes.");
        }
    ...
    public function checkColumnDefinition(array $a_def): bool
    {
        ...
        if (!in_array($a_def["type"], $this->getAvailableTypes(), true)) {
            switch ($a_def["type"]) {
                case "boolean":
                    throw new Exception("Invalid column type '" . $a_def["type"] . "'. Use integer(1) instead.");

                case "decimal":
                    throw new Exception("Invalid column type '" . $a_def["type"] . "'. Use float or integer instead.");

                default:
                    throw new Exception("Invalid column type '" . $a_def["type"] . "'. Allowed types are: "
        ...
        foreach (array_keys($a_def) as $k) {
            if ($k !== "type" && !in_array($k, $allowed_attributes[$a_def["type"]], true)) {
                throw new Exception("Attribute '" . $k . "' is not allowed for column type '" . $a_def["type"] . "'.");
            }
        }
        ...
        switch ($a_def["type"]) {
            case self::T_TEXT:
                if ((!isset($a_def["length"]) || $a_def["length"] < 1 || $a_def["length"] > $max_length[self::T_TEXT]) && isset($a_def["length"])) {
                    throw new Exception("Invalid length '" . $a_def["length"] . "' for type text." . " Length must be >=1 and <= "
            ...
            case self::T_INTEGER:
                if (isset($a_def["length"]) && !in_array((int) $a_def["length"], $max_length[self::T_INTEGER], true)) {
                    throw new Exception("Invalid length '" . $a_def["length"] . "' for type integer." . " Length must be "
                        . implode(', ', $max_length[self::T_INTEGER]) . " (bytes).");
                }
                if ($a_def["unsigned"] ?? null) {
                    throw new Exception("Unsigned attribut must not be true for type integer.");
                }
                break;
        }
    ...
    protected function getDBInstance(): DbPdo
    {
    ...
    public function compareDefinition(array $current, array $previous): array
    {
        ...
        if (!method_exists($this, "compare{$type}Definition")) {
            ...
            throw new Exception('type "' . $current['type'] . '" is not yet supported');
        }
    ...
    public function writeLOBToFile($lob, string $file): bool
    {
        ...
        while (!@feof($lob)) {
            ...
            if (@fwrite($fp, $result, $read) !== $read) {
                ...
                throw new Exception('could not write to the output file');
            }
        }
        @fclose($fp);
    ...
    public function matchPattern(array $pattern, $operator = null, $field = null): string
    {
        ...
        if (!is_null($operator)) {
            ...
            switch ($operator) {
                ...
                case 'ILIKE':
                    if (is_null($field)) {
                        throw new Exception('case insensitive LIKE matching requires passing the field name');
                    }
                ...
                case 'LIKE':
                    ...
                    break;
                default:
                    throw new Exception('not a supported operator type:' . $operator);
            }
        }
        $match .= "'";
