...
$ecs_local_functions = new EcsLocalFunctions();

$app->get(SLASH, function (Request $request, Response $response) use ($logging, $ecs_local_functions) {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
...
$app->get(SYS_EVENTS_FIFO, function (Request $request, Response $response) use ($db, $logging, $ecs_local_functions)  {
    $valid_participant = $ecs_local_functions->isValidParticipant();
    ...
    if(count($data) > 0) {
        foreach ($data as $row) {
            if (isset($row['service_id']) && $row['service_id'] > 0 && isset($row['cmd'])) {
                $value[] = $ecs_local_functions->convertEventsFifoData($row);
            }
        }
    ...
    $json = $ecs_local_functions->jsonEncodeWrapper($value);
    $response->getBody()->write(print_r($json, true));

    return $response->withHeader('Content-Type', 'application/json');
});

$app->post(SYS_EVENTS_FIFO, function (Request $request, Response $response) use ($db, $logging, $ecs_local_functions)  {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
...
$app->get(SYS_MEMBERSHIPS, function (Request $request, Response $response) use ($db, $logging, $ecs_local_functions)  {
    $valid_participant = $ecs_local_functions->isValidParticipant();
    if($valid_participant->isInvalidAuth()) {
        return $response;
    }

    $memberships_json = $ecs_local_functions->jsonEncodeWrapper($ecs_local_functions->getMemberships($valid_participant));
    $response->getBody()->write($memberships_json);

    return $response->withHeader('Content-Type', 'application/json');
});

$app->get(CC_COURSES, function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions)  {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
...
$app->get('/campusconnect/courses/{id}/details', function (Request $request, Response $response, $args) use ($db, $logging, $ecs_local_functions)  {
    $valid_participant = $ecs_local_functions->isValidParticipant();
    if($valid_participant->isInvalidAuth()) {
        return $response;
    }
...
$app->get('/campusconnect/course_members', function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions) {
    ...
    $data = $db->selectQueueWaitingEntries($ecs_local_functions->isValidParticipant()->getPid(), COURSE_MEMBERS);
    $json = $ecs_local_functions->jsonEncodeWrapper($data);
    $response->getBody()->write($json);
    return $response->withHeader('Content-Type', 'text/uri-list');
});

$app->get('/campusconnect/course_members/[{id}]', function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions) {
    if ($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
...
$app->get('/campusconnect/categories/{id}/details', function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions)  {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
    return $response;
...
$app->get(CC_COURSE_LINKS, function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions)  {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
...
$app->post(CC_COURSE_URLS, function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions) {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
    if(GlobalSettings::getInstance()->isCreateLinks()) {
        ...
        if(is_array($post)) {
            $course_urls = $ecs_local_functions->parseCourseUrls($post);
...
$app->get(CC_COURSE_URLS, function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions) {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }

    return $ecs_local_functions->echoReturnFunction([], $response)->withHeader('Content-Type', 'text/uri-list');
});

$app->get('/campusconnect/course_urls/[{id}]', function (Request $request, Response $response, array $args) use ($db, $logging, $ecs_local_functions) {
    if($ecs_local_functions->isValidParticipant()->isInvalidAuth()) {
        return $response;
    }
